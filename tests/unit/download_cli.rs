...
/// US3 Test: Verify --metrics flag defaults to false (Feature 003)
#[test]
fn test_cli_metrics_defaults_to_false() {
    // Parse CLI with minimal args (no --metrics flag)
    let args = vec![
        "trading-data-downloader",
        "download",
        "bars",
    ...
    // Verify metrics defaults to false (disabled by default per FR-004)
    assert_eq!(
        cli.metrics, false,
        "Metrics should be disabled by default per FR-004"
    );
}

/// US3 Test: Verify --metrics flag can be enabled (Feature 003)
#[test]
fn test_cli_respects_metrics_flag() {
    // Parse CLI with --metrics flag
    let args = vec![
        "trading-data-downloader",
        "--metrics",
        "download",
    ...
    // Verify metrics is enabled when --metrics flag is provided
    assert_eq!(
        cli.metrics, true,
        "Metrics should be enabled when --metrics flag is provided"
    );
}
