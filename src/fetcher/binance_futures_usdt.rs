...
const AGGTRADES_ENDPOINT: &str = "/fapi/v1/aggTrades";
...
impl BinanceFuturesUsdtFetcher {
    ...
    pub fn new() -> Self {
        Self {
            client: Client::new(),
            base_url: BINANCE_FAPI_BASE_URL.to_string(),
    ...
    /// Parse a single kline array from API response (T056)
    ...
    /// Parse a single aggTrade object from API response (T118)
    ...
    /// Parse a single funding rate from API response (T142)
...
mod tests {
    ...
    fn test_parse_kline() {
        ...
        let bar = BinanceFuturesUsdtFetcher::parse_kline(&kline_json).unwrap();
